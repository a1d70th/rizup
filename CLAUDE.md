...
- `public/dashboard.html`：`<h1>Lueur Inc. — 経営ダッシュボード</h1>` の上に `<img src="/logo-company.svg">` を追加、h1 を「経営ダッシュボード」のみに簡素化
...
## 本人
児玉翔平・28歳・大阪・資金200万・Lueur Inc.（リュエール）代表
今月目標：20万円 / 来月目標：30万円

---

## 🏢 Lueur Inc. 事業ポートフォリオ（2026-04-18 確定・3事業体制）

| # | 事業 | 状態 | 収益モデル | 担当 |
|---|---|---|---|---|
| ① | **Rizup** | PHASE1 最優先・デプロイ済 | Free + Pro ¥480/月 | Sora (CTO) |
| ② | **HARU**（日韓特化・大阪発 飲み会／🌸 하루）| PHASE1 同時着手・立上中 | ¥2,500/回 → Airbnb Experience 移行で ¥100k+/月 | 翔平さん 直轄 |
| ③ | **KAKEHASHI**（在日外国人 生活サポート） | PHASE2 転職後着手 | ¥5,000〜10,000/回 → ¥30-100k/月/社（BtoB） | 翔平さん 直轄 |

**事業②の経緯**: 当初 NOMI（日本人×外国人 汎用）で構想 → 2026-04-18 に **HARU（日韓特化）** へピボット。翔平さんが韓国語学習中・韓国人友達 1 人アサイン済・Airbnb Experience 展開見据え。詳細: `company/new-business/HARU/`（STRATEGY / TEAM_REVIEW / EVENT_COPY / INSTAGRAM_STRATEGY の 4 本）。

**Team Review 結果（2026-04-18）**: 6 社員平均 23.3/40・**🟡 修正後 GO**。Phase 1 開始条件 = ①女性共同ホスト内諾 ②商標調査 ③Rizup 本業の SQL migration + 環境変数完了。

**撤去した事業**: AI×採用コンサル / チームビルディング研修 / 強み診断AI / 転職×メンタルコーチング / 英語×AI活用スクール / Rizup for Business / Rizup Global（全て 0〜10% 未検証仮説だったため切り捨て、3事業に集中）。

**詳細**: `company/new-business/NOMI_KAKEHASHI.md` に NOMI / KAKEHASHI のフェーズ設計・収益試算・週時間割あり。

### 新規事業の位置付け
- **Rizup** = プロダクト（PWA / 朝夜ジャーナル / 国内ベータ）
- **NOMI** = リアルイベント事業（翔平さんが週 1 ホスト参加）
- **KAKEHASHI** = BtoB / 副業サポート（転職先が決まった後）
- 全て Lueur Inc. の傘下で独立事業として並走

---

## 💎 Rizup コミュニティ特典設計（2026-04-18 確定版）

### Free（¥0）
- 全機能使い放題（朝夜ジャーナル・タイムライン・リアクション・ビジョン・習慣）
- 広告あり（自前ハウス広告）
- **月 1 オンラインミーティング 参加可**（初期は全員無料開放）
...
### 🎥 月 1 オンラインミーティング 方針（2026-04-18 再確定）
- **当面は Free / Pro 両方が参加可能**（最初は無料開放で母数を作る）
- 実績がついたら **Pro 限定化を検討**（参加制限を段階的に）
- 録画なし・Zoom or Google Meet・翔平さんがホスト

### 🍻 オフ会方針（2026-04-18 再確定）
- **まだまだ先**。「やるかも」程度のスタンス
- 日程・料金 **未定**
- 公開面での言及は **「不定期で開催予定」程度** に抑える（具体金額は明示しない）

### Premium（¥1,980/月・Phase2 将来）
- Pro の全特典
- オフ会参加費無料
- 翔平さんとの Q&A 権
- 限定 Discord アクセス

**運用ルール**: Premium 公開は Pro 50 人到達後。それまでは Free + Pro のみ表示。

### イベント実装フェーズ
- **Phase1（今月）**: Google フォーム + スレッズ告知 + メール送信で運用（軽量）
- **Phase2（来月）**: `/events` ページをアプリに実装・Pro ユーザーのみボタン表示・`events` テーブル追加

詳細: `company/events/COMMUNITY_EVENTS.md`

### 🚦 フィーチャーフラグ: `NEXT_PUBLIC_ENABLE_MEETING`

- **デフォルト: `false`**（ローンチ時は月1MTGカードを「近日公開」表示にし、フィードバック送信 CTA だけ出す）
- **`true` にするだけで** `/events` から参加登録フォームへの遷移が解禁される
- **解禁条件（どちらか満たしたら true に切り替え）**:
  1. **フィードバック 5 件受信**（ユーザーの声が集まった）
  2. **Pro 10 人達成**（継続ユーザーが見えてきた）
- 切替手順: Vercel → Project → Settings → Environment Variables に `NEXT_PUBLIC_ENABLE_MEETING=true` を追加 → Redeploy。コード変更不要。
