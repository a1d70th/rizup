...
export default function LoginPage() {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (error) {
        console.error("[Rizup Login] Google OAuth error:", error.message);
        showError("Google ログインの開始に失敗しました。もう一度お試しください。");
      }
    } catch (err) {
      console.error("[Rizup Login] Google OAuth catch:", err);
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col items-center justify-center px-6 py-12">
      ...
      <div className="w-full max-w-xs flex flex-col gap-3">
        ...
        {error && (
          <div className="bg-red-50 border border-red-200 rounded-2xl p-3 animate-fade-in" role="alert">
      ...
      <div className="w-full max-w-xs flex flex-col gap-3">
        ...
        <button
          type="button"
          onClick={handleGoogleLogin} aria-label="Googleでログイン"
